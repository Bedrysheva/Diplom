...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	Если ИмяФормы = "Справочник.ДоговорыКонтрагентов.Форма.ФормаЭлемента" Тогда
		ВКМ_АбонентскоеОбслуживание.ВывестиДополнительныеПоляАбонентскогоДоговора(Форма);
	КонецЕсли;	
	
	Если ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаДокумента" Тогда
		ВКМ_АбонентскоеОбслуживание.ВывестиКнопкуЗаполнитьПоДоговоруАбонентскогоОбслуживания(Форма);
	КонецЕсли;	
	//-- Бедрышева Н.В. Задача "Абонентское обслуживание" 08.06.2025
